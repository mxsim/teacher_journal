...
router.post("/add", async (req, res) => {
  try {
    ...
    const query = `
      ...
      JOIN lessons l ON u.name = ? AND u.user_surname = ? AND u.user_parent_name = ?
  ...
  try {
    ...
    const query = `
      ...
      WHERE student_id = (
          ...
          FROM students s
          JOIN users u ON s.user_id = u.user_id
          WHERE u.name = ? AND u.user_surname = ? AND u.user_parent_name = ?
      )
      AND lesson_id = (
          SELECT l.lesson_id
          FROM lessons l
...
router.get("/get_students", (req, res) => {
  ...
  try {
    const sql = `
      SELECT CONCAT(u.name, ' ', u.user_surname, ' ', u.user_parent_name) AS student
